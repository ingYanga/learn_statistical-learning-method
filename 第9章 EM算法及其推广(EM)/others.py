...
def M_step(muo, mu1, gamma0, gamma1, dataSetArr):
    ...
    sigmod0_new = math.sqrt(
        np.dot(gamma0, (dataSetArr - muo)**2) / np.sum(gamma0))
    sigmod1_new = math.sqrt(
        np.dot(gamma1, (dataSetArr - mu1)**2) / np.sum(gamma1))
...
def EM_Train(dataSetList, iter=500):
    ...
    while (step < iter):
        ...
        gamma0, gamma1 = E_step(dataSetArr, alpha0, mu0,
                                sigmod0, alpha1, mu1, sigmod1)
        #步骤3：M步
