...
class HiddenMarkov:
    ...
    def backward(self, Q, V, A, B, O, PI):  # 后向算法
        ...
        P = np.dot(np.multiply(PI, [b[indexOfO]
                                    for b in B]), [beta[0] for beta in betas])
        ...
        for t in range(M):
            ...
            for i in range(N):
                ...
                if t == 0:
                    ...
                    deltas[i][t] = np.max(np.multiply(
                        [delta[t-1] for delta in deltas], [a[i] for a in A])) * B[i][indexOfO]
                    print('delta%d(%d)=max[delta%d(j)aj%d]b%d(o%d)=%.2f*%.2f=%.5f' % (realT, realI, realT-1, realI, realI, realT, np.max(
                        np.multiply([delta[t-1] for delta in deltas], [a[i] for a in A])), B[i][indexOfO], deltas[i][t]))
                    psis[i][t] = np.argmax(np.multiply(
                        [delta[t-1] for delta in deltas], [a[i] for a in A]))
